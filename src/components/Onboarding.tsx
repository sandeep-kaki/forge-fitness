...
export function Onboarding({ onComplete }: OnboardingProps) {
  ...
  return <main className="onboarding"><header className="onboard-top"><div className="wordmark"><span>F</span>FORGE</div><span className="onboard-count">{step + 1} / 8</span></header><div className="onboard-progress"><i style={{ width: `${((step + 1) / 8) * 100}%` }} /></div><section className="onboard-content">
    ...
    {step === 6 && <><p className="eyebrow">YOUR EXPERIENCE</p><h1>Where are you starting from?</h1><p>We'll keep guidance clear and avoid assumptions about what you can safely lift.</p><div className="choice-list">{(['beginner', 'intermediate', 'advanced'] as const).map((level) => <button className={experience === level ? 'choice selected' : 'choice'} key={level} onClick={() => setExperience(level)}><span><strong>{level[0].toUpperCase() + level.slice(1)}</strong><small>{level === 'beginner' ? 'I want step-by-step guidance.' : level === 'intermediate' ? 'I have experience training consistently.' : 'I push hard and know my limits well.'}</small></span><b>{experience === level ? '✓' : ''}</b></button>)}</div></>}
  ...
  </section><footer className="onboard-footer">{step > 0 ? <button className="back-button" onClick={previous}>Back</button> : <span />}<div className="onboard-footer-end">{step === 7 && !canContinue && <p className="onboard-hint">Tick both boxes above to continue.</p>}{step === 7 ? <button className="button primary" disabled={!canContinue} onClick={finish}>Create my profile</button> : <button className="button primary" disabled={!canContinue} onClick={next}>Continue</button>}</div></footer></main>
}
